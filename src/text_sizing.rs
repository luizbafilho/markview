...
pub fn extract(
    ...
    for (src_row, mut line) in std::mem::take(lines).into_iter().enumerate() {
        let level = line.spans.first_mut().and_then(|first| {
            let level = match first.content.chars().next()? {
                H1 => 1,
                H2 => 2,
                H3 => 3,
                _ => return None,
...
#[expect(
    ...
    reason = "x/y are cell coordinates; n/d/w mirror the OSC 66 keys"
)]
pub fn place(
    line: &Line,
    level: u8,
    ...
    max_width: u16,
    base: Style,
) -> Result<Option<Placed>, fmt::Error> {
    let mut escape = String::new();
    for span in &line.spans {
        sgr(&mut escape, base.patch(line.style).patch(span.style))?;
        match fraction(level) {
            None => write!(escape, "\x1b]66;s=2;{}\x07", span.content)?,
            Some((n, d)) => {
                for (chunk, cols) in chunks(&span.content, d) {
                    let w = (n * cols).div_ceil(d);
                    write!(escape, "\x1b]66;s=2:n={n}:d={d}:v=2:w={w};{chunk}\x07")?;
                }
            }
        }
    }
    SetAttribute(Attribute::Reset).write_ansi(&mut escape)?;
    ResetColor.write_ansi(&mut escape)?;
    let width = scaled_width(line, level);
    Ok((width <= max_width).then_some(Placed {
        x,
        y,
        width,
        rows: ROWS,
...
pub fn scale(level: u8) -> f32 {
    fraction(level).map_or(2.0, |(n, d)| 2.0 * f32::from(n) / f32::from(d))
}
...
fn scaled_width(line: &Line, level: u8) -> u16 {
    line.spans
        ...
        .map(|span| match fraction(level) {
            ...
            Some((n, d)) => chunks(&span.content, d)
                .iter()
                .map(|(_, cols)| 2 * (n * cols).div_ceil(d))
                .sum(),
        })
        .sum()
}
...
fn wrap(line: &Line<'static>, level: u8, max: u16) -> Vec<Line<'static>> {
    ...
    while start < chars.len() {
        let mut end = start;
        let mut space = None;
        let is_space = |i: usize| chars.get(i).is_some_and(|&(c, _)| c == ' ');
        while let Some(candidate) = chars.get(start..=end)
            && scaled_width(&row(candidate), level) <= max
        {
            if is_space(end) {
                space = Some(end);
            }
        ...
        let mut cut = end;
        while cut > start && is_space(cut - 1) {
            cut -= 1;
        }
        rows.push(row(chars.get(start..cut).unwrap_or_default()));
        start = end;
...
pub fn rule(x: u16, y: u16, width: u16, style: Style) -> Result<Placed, fmt::Error> {
    let mut escape = String::new();
    sgr(&mut escape, style)?;
    let mut left = width;
    while left > 0 {
        let w = left.min(7);
        let line = "─".repeat(4 * w as usize);
        write!(escape, "\x1b]66;n=1:d=4:w={w};{line}\x07")?;
        left -= w;
    }
    SetAttribute(Attribute::Reset).write_ansi(&mut escape)?;
    ResetColor.write_ansi(&mut escape)?;
...
fn chunks(s: &str, cols: u16) -> Vec<(String, u16)> {
    ...
    for ch in s.chars() {
        ...
        match out.last_mut() {
            Some((text, used)) if *used + w <= cols => {
                text.push(ch);
                *used += w;
            }
            _ => out.push((ch.to_string(), w)),
        }
    }
    out.retain(|(_, c)| *c > 0);
    out
...
fn sgr(out: &mut String, style: Style) -> fmt::Result {
    SetAttribute(Attribute::Reset).write_ansi(out)?;
    if let Some(fg) = style.fg {
        SetForegroundColor(fg.into()).write_ansi(out)?;
    }
    if let Some(bg) = style.bg {
        SetBackgroundColor(bg.into()).write_ansi(out)?;
    }
    for (m, a) in [
...
mod tests {
    ...
    fn h1_wider_than_the_viewport_wraps_into_rows_that_all_fit() {
        ...
        for h in &headings {
            assert!(
                place(&lines[h.row], 1, 0, 0, 108, Style::new())
                    .unwrap()
                    .is_some(),
                "row {} does not fit",
                h.row
    ...
    fn h3_is_tagged_and_drawn_at_one_and_a_quarter_scale() {
        ...
        tag(&mut blocks);
        let MarkdownBlock::Heading3(t) = &blocks[0] else {
            unreachable!()
        };
        ...
        let p = place(&lines[headings[0].row], 3, 0, 0, 80, Style::new())
            .unwrap()
            .unwrap();
    ...
    fn only_h1_and_h2_get_a_rule() {
        ...
        assert_eq!(
            headings.iter().map(|h| h.row).collect::<Vec<_>>(),
            [0, 3, 6]
        );
        assert_eq!(rules, [2, 5]);
    }
