...
pub fn transmit(img: &DynamicImage, id: u32, cols: u16, rows: u16, tmux: bool) -> String {
    let (start, esc, end) = if tmux { ("\x1bPtmux;", "\x1b\x1b", "\x1b\\") } else { ("", "\x1b", "") };
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        write!(out, "{esc}_Gq=2,").unwrap();
        if i == 0 {
            write!(out, "i={id},a=T,U=1,f=32,t=d,s={},v={},c={cols},r={rows},", img.width(), img.height()).unwrap();
        }
        let more = u8::from(i + 1 < chunks.len());
        write!(out, "m={more};{}{esc}\\", base64_simd::STANDARD.encode_to_string(chunk)).unwrap();
    }
    out.push_str(end);
    out
}
...
pub fn cell(row: u16, col: u16) -> String {
